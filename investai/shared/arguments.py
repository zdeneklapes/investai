...
def parse_arguments() -> Tuple[vars, Namespace]:
    ...
    parser.add_argument(
        "--wandb-model-save-freq", help="Save model every x steps (0 = no checkpoint)", type=int, default=100
    )
    parser.add_argument(
        "--wandb-gradient-save-freq", help="Save gradient every x steps (0 = no checkpoint)", type=int, default=100
    )
    parser.add_argument(
        "--wandb-verbose", help="Verbosity level 0: not output 1: info 2: debug, default: 0", type=int, default=0
    )

    # W&B Environment variables
    parser.add_argument("--wandb-entity", help="Wandb entity", default=os.environ.get("WANDB_ENTITY", None))
    parser.add_argument(
        "--wandb-project", help="Wandb project name", default=os.environ.get("WANDB_PROJECT", "portfolio-allocation")
    )
    ...
    parser.add_argument(
        "--torch-deterministic", help="if toggled, `torch.backends.cudnn.deterministic=False`", action="store_true"
    )
    parser.add_argument(
        "--track", help="if toggled, this experiment will be tracked with Weights and Biases", action="store_true"
    )
    parser.add_argument(
        "--capture-video",
    ...
    # Algorithm arguments
    parser.add_argument("--env-id", help="the id of the environment", type=str, default="")
    ...
    parser.add_argument("--policy-frequency", help="the frequency of training policy (delayed)", type=int, default=2)
    ...
    parser.add_argument(
        "--n-steps", help="the number of steps to run in each environment per update", type=int, default=20
    )
    parser.add_argument("--gamma", help="the discount factor gamma", type=float, default=0.99)
    parser.add_argument(
        "--gae-lambda", help="the lambda for the general advantage estimation", type=float, default=0.95
    )
    ...
    parser.add_argument("--max-grad-norm", help="the maximum value for the gradient clipping", type=float, default=0.5)
    parser.add_argument(
        "--rms-prop-eps", help="RMSProp optimizer epsilon (stabilizes square root)", type=float, default=1e-5
    )
    parser.add_argument("--use-rms-prop", help="whether to use RMSProp (otherwise, use Adam)", action="store_true")
    parser.add_argument("--use-sde", help="whether to use generalized State Dependent Exploration", action="store_true")
    ...
    parser.add_argument(
        "--tensorboard-log", help="the log location for tensorboard (if None, no logging)", type=str, default=None
    )
    # parser.add_argument(
    #     "--policy-kwargs",
    ...
    # )
    parser.add_argument(
        "--verbose", help="the verbosity level: 0 none, 1 training information, 2 tensorflow debug", type=int, default=0
    )
    ...
    parser.add_argument(
        "--learning-starts", help="Number of steps before learning for the warm-up", type=int, default=1000
    )
    ...
    parser.add_argument(
        "--optimize-memory-usage", help="Enable a memory efficient variant of the replay buffer", action="store_true"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--use-sde-at-warmup", help="Use gSDE instead of normal action noise at warmup", action="store_true"
    )
    ...
    parser.add_argument(
        "--exploration-initial-eps", help="Initial value of random action probability", type=float, default=1.0
    )
    parser.add_argument(
        "--exploration-final-eps", help="Final value of random action probability", type=float, default=0.02
    )
