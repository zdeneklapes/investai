...
% RL in Portfolio Allocation


% Portfolio Allocation
The portfolio allocation is a problem that has been studied for a long time. The
problem is to find the optimal allocation of the portfolio, given a
specific set of securities.

The portfolio management task is basically the most important to the
finance institutions, like banks, hedge funds and asset management companies.
It help traders understand the market and its risks.\cite{finrl-portfolio-allocation-2020}

The portfolio allocation was widely studied in a standart ways, like the
Markowitz portfolio theory\cite[p.~4]{sirucek-2015}, the CAPM model and the Black-Litterman model.

The stock market is a wide and quick changing environment and people are
emotionally based, which means, that behave based on their emotions and only
a few people can do rational decisions. This could be problem, specifically
when you need to decide based on what you know about given asset.

% RL all
In this thesis, I will focus on the application of RL in the field
of finance, specifically in
portfolio allocation. The goal of portfolio allocation is to find the optimal
portfolio of assets that maximizes the expected return for a given level of
risk.\cite{finrl-portfolio-allocation-2020}

Deep Reinforcement Learning (DRL) methods have been used in many different
fields, such as robotics, video games, and finance. And studied how much a change
in input would influence the output is important to understand what
contributes to the decision making process of the DRL agents.\cite{finrl-portfolio-allocation-2020}
...
\begin{itemize}
    \item Value Based: $V(s,\theta)$ \cite{sutton-2018}
