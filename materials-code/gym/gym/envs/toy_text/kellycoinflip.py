...
class KellyCoinflipEnv(gym.Env):
    ...
    def __init__(self, initialWealth=25.0, edge=0.6, maxWealth=250.0, maxRounds=300):
        ...
        self.action_space = spaces.Discrete(
            int(maxWealth * 100)
        )  # betting in penny increments
        self.observation_space = spaces.Tuple(
            (spaces.Box(0, maxWealth, [1]), spaces.Discrete(maxRounds + 1))  # (w,b)
        )
    ...
    def step(self, action):
        ...
        if (
            action > self.wealth
        ):  # treat attempts to bet more than possess as == betting everything
            action = self.wealth
...
class KellyCoinflipGeneralizedEnv(gym.Env):
    ...
    def __init__(
        ...
        maxWealth = round(
            genpareto.rvs(maxWealthAlpha, maxWealthM, random_state=prng.np_random)
        )
    ...
    def _get_obs(self):
        return (
            ...
            self.wins,
            self.losses,
            np.array([float(self.maxEverWealth)]),
        )
    ...
    def reset(self):
        ...
        self.__init__(
            initialWealth=self.initialWealth,
            edgePriorAlpha=self.edgePriorAlpha,
            edgePriorBeta=self.edgePriorBeta,
            maxWealthAlpha=self.maxWealthAlpha,
            maxWealthM=self.maxWealthM,
            maxRoundsMean=self.maxRoundsMean,
            maxRoundsSD=self.maxRoundsSD,
            reseed=False,
        )
        return self._get_obs()
